...
int v0 = 0; // Medicion de voltaje
float vi = 0.0; // Medicion tras multiplicar por LAMBDA
...
void interrupt(){
    ...
    if(INTCON.TMR0IF == 1){
        ...
        // Realizar medicion tras el segundo
        ADCON0.ADON = 1;
        ADCON0.GO = 1;
...
void main(){
    ...
    // Configurar el timer (Desactivado en un primer momento)
    T0CON = 0x84;
    TMR0H = (ALFA >> 8);
    TMR0L = ALFA;

    // Activar interrupciones del timer
    INTCON.TMR0IF = 0;
    INTCON.TMR0IE = 1;
